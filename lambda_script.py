...
def remove_song_anatomy_tags(l):
    ...
    return f"""{normalize_text(' '.join(edited))}"""
...
def get_song_lyrics_and_features(track_id, track_name, artist_name, popularity=50, explicit=0):
    ...
    try:
        ...
        print('oops no lyrics fo')
        lyrics = f'{track_name} {artist_name}'
    # print(lyrics)
...
def lambda_handler(event, context):
    ...
    for song in event:
        ...
        response = sagemaker_runtime.invoke_endpoint(
                    ...
                    Body=json.dumps({"inputs":full_lyrics[:1024]}),
    ...
    return {
        'statusCode': 200,
        'body': json.dumps(f'success for this batch')
    }
