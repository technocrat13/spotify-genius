...
def remove_song_anatomy_tags(l):
    ...
    return normalize_text(' '.join(edited))
...
def lambda_handler(event, context):
    ...
    for i, song in enumerate(event):
        ...
        response = sagemaker_runtime.invoke_endpoint(
                    EndpointName='huggingface-pytorch-inference-2023-06-10-20-05-18-001',
                    Body=json.dumps({"inputs":full_lyrics}),
                    ContentType='application/json',
        )

        # Parse the response
        predictions = json.loads(response['Body'].read().decode())[0][1]
        # print(predictions)
        
        embeddings_proj = np.dot(R, predictions)
        # embeddings_proj = matmul(R, embeddings)

        features.extend(embeddings_proj)
        print(features)
        data = {
            'name': track_name,
            'artist': artist_name,
            'features': features,
            'lyrics': full_lyrics,
            'full_embedding': predictions
        }
        
        s3.put_object(Body=json.dumps(data), Bucket=bucket_name, Key=f'{key}/{track_id}_{track_name}_{artist_name}.json')
        print(f'success for {track_name} by {artist_name}')
        
        if i == 0:
            first_song = f'{track_name} by {artist_name}'
            
        count = i
    last_song = f'{track_name} by {artist_name}'
        
    return {
        'statusCode': 200,
        'body': json.dumps(f'success for {count} songs from {first_song} to {last_song}')
    }
